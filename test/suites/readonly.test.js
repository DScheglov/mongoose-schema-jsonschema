...
describe('readonly: model.jsonSchema', () => {
  it('should consider the readonly parameter', () => {
    const rules = [
      {
        name: '**_id',
        path: /^([^.]?\.?)*_id$/,
    ...
    const jsonSchema = models.Book.jsonSchema(null, null, rules);
  ...
  it('should exclude comment.editor', () => {
    const rules = [
      {
        name: '**_id',
        path: /^([^.]?\.?)*_id$/,
