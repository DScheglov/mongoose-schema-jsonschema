...
function __populate(jsonSchema, populate) {
  ...
  for (path of pTree) {
    ...
    model = jss.spec['x-ref'];
    if (!model) continue;
    try {
      model = path.model || mongoose.model(model);
    } catch(e) {
      continue;
    }
...
function __select(schema, jsonSchema, fields) {
  var excludePath = normalizeSelection(schema, fields);
  var path, jss;
  for (path of excludePath) {
    jss = jssPath(jsonSchema, path);
    if (!jss.baseCont) continue;
...
function normalizeSelection(schema, selection) {
  ...
  if (typeof(selection) === 'string') {
    selection = selection.split(/\s+/).reduce( (sel, p) => {
      if (p[0] === '+') {
        delete exclude[p];
        p = p.substr(1);
      }
