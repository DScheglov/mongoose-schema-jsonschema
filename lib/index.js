...
var findPath = require('./jss-path').findPath;
var ensurePath = require('./jss-path').ensurePath;
...
function schema_jsonSchema(name) {
  this.__jsonSchema = this.__jsonSchema || build(name, this);
...
function __excludedPaths(schema, selection) {
  ...
  if (explicitlyInclude) {
    ...
    return paths.filter(p => {
      var f = !selection[p];
      var g = Object.keys(selection).some(s => p.indexOf(s+'.') === 0 && selection[s]);
      // if (selection[p] != null) {
      //   g = !selection[p];
      // }
      return f && !g;
